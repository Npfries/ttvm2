...
class Queue {
  ...
  async init() {
    ...
    await ch1.assertQueue(TASKS_QUEUE_NAME);

    this.pushChannel = await this.connection.createChannel();
    this.pushChannel.assertQueue(RESULTS_QUEUE_NAME);

    ch1.prefetch(1);

    // Listener
    ch1.consume(TASKS_QUEUE_NAME, (msg) => {
  ...
  push(msg) {
    ...
    try {
      ...
      this.pushChannel.sendToQueue(RESULTS_QUEUE_NAME, buffer);
    } catch (e) {
      console.log("unable to convert msg to string");
    }
  }
}
